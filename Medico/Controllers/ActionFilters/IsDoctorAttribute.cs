...
namespace Medico.Controllers.ActionFilters
{
    public class IsDoctorAttribute : ActionFilterAttribute
    {
        public override void OnActionExecuting(HttpActionContext actionContext)
        {
            ...
            var isUserAdmin = UserService.GetByUserIdType(user_id, "Doctor" ) || UserService.GetByUserIdType(user_id, "Admin");

            if (!isUserAdmin)
            {
                actionContext.Response = actionContext.Request.CreateErrorResponse(HttpStatusCode.Unauthorized, "User not Doctor or Higher user");
                return;
            }
        }
    }
